...
a2560k_make :: proc(name: string, pic: ^pic.PIC) -> ^Bus {
    ...
    b.pic     = pic
    b.read    = a2560k_read
    b.write   = a2560k_write

    ebus = b
    return b
}

a2560k_read :: proc(bus: ^Bus, size: emu.Request_Size, addr: u32) -> (val: u32) {
    ...
    case                              :  a2560k_bus_error(bus, "read", size, addr)
    }
...
a2560k_write :: proc(bus: ^Bus, size: emu.Request_Size, addr, val: u32) {
    //spall.SCOPED_EVENT(&spall_ctx, &spall_buffer, #procedure)
    ...
    case 0xFF_C0_0000 ..= 0xFF_FF_FFFF:  emu.not_implemented(#procedure, "flash0", size, addr)
    case                              :  a2560k_bus_error(bus, "write", size, addr)
    }

    return
}

a2560k_bus_error :: proc(b: ^Bus, op: string, size: emu.Request_Size, addr: u32) -> u8 {
    log.errorf("%s err %5s%d    at 0x %04X:%04X - a2560k unknown segment", b.name, op, size, u16(addr >> 16), u16(addr & 0x0000_ffff))
    return 0
}
